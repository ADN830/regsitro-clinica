...
<head>
	...
	//crear header en archivo externo
...
<body>
	<div data-role="page" class="ui-responsive-panel" id="e_analisis" data-dialog="true">
		...
		//la las variable no se va a usar en varias ocacion, no crear aumenta el consumo de momiria
		$id = $_GET['id'];

		$Rpop = $db->prepare(
			"SELECT a.*, coalesce(ma.nombre_base, '-') nombre_base, coalesce(ma.nombre_original, '-') nombre_original
			FROM hc_analisis a
			left join man_archivo ma on ma.id = a.archivo_id
			where a.id = ?;"
		);
		//esto funciona igual y reduce el consumos de momiria
		//$Rpop->execute(array($_GET['id']));
		...
		<div data-role="header" data-theme="b" data-position="fixed">
			...
			<h1>Nuevo <?php if ($login == 'eco') echo 'Ecografía';
						else echo 'Exámen'; ?></h1>
		</div>
		...
		<div class="ui-content" role="main">
			<form action="" method="post" enctype="multipart/form-data" data-ajax="false" id="form1">
				...
				<table width="100%" align="center" style="margin: 0 auto;">
						...
						<td colspan="2">
							<select name="a_exa" id="a_exa" required data-mini="true">
								...
								while ($med = $rMed->fetch(PDO::FETCH_ASSOC)) { ?>
									<option value="<?php echo $med['nom']; ?>" <?php if ($med['nom'] == $pop['a_exa']) echo 'selected'; ?>><?php print(mb_strtoupper($med['nom'])); ?></option>
								<?php } ?>
							</select>
						</td>
					</tr>
						...
						<tr>
							...
							<td colspan="2">
								<select name="a_sta" id="a_sta" required data-mini="true">
									<option value="">SELECCIONAR</option>
									<option value="Positivo" <?php if ($pop['a_sta'] == 'Positivo') print('selected'); ?>>POSITIVO</option>
									<option value="Negativo" <?php if ($pop['a_sta'] == 'Negativo') print('selected'); ?>>NEGATIVO</option>
					...
					<tr>
						...
						<td colspan="2">
							...
							<?php
							if (file_exists('analisis/' . $pop['id'] . '_' . $pop['a_dni'] . '.pdf')) {
								print('<em><a href="analisis/' . $pop['id'] . '_' . $pop['a_dni'] . '.pdf' . '" target="new" style="margin: .446em; font-size: 12px;">Ver Informe</a></em>');
							} ?>
						</td>
					</tr>
					...
					<tr>
						...
						<td colspan="2">
							...
							<?php
							$link_video = '';
							$stmt = $db->prepare("SELECT * from google_drive_response where drive_id <> 0 and estado = 1 and tipo_procedimiento_id = 2 and procedimiento_id = ? order by id desc limit 0, 1;");
							$stmt->execute([$pop['id']]);
							if ($stmt->rowCount() > 0) {
					...
					if ($login == "eco") {
						...
						$stmt = $db->prepare("SELECT `user` codigo, upper(nom) nombre from usuario u where `role` = 1 order by nom;");
						$stmt->execute();
					...
					<tr>
						<td>Observación</td>
						<td colspan="2">
							<textarea name="a_obs" id="a_obs" data-mini="true"><?php echo $pop['a_obs']; ?></textarea>
						</td>
					</tr>
				</table>
				...
				<div class="enlinea">
					...
					<?php if ($login == 'eco') {
						echo '<input type="hidden" name="cor" id="cor">';
					} else { ?>
