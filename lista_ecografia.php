...
<head>
	...
	if (!$login) {
		print("<META HTTP-EQUIV='Refresh' CONTENT='0; URL=http://" . $_SERVER['HTTP_HOST'].substr(dirname(__FILE__), strlen($_SERVER['DOCUMENT_ROOT'])) . "'>");
	}
...
<body>
	<?php
	$stmt = $db->prepare("SELECT role, sede_id FROM usuario WHERE user=?");
			...
			<form action="" method="post" data-ajax="false" name="form1" id="form1">
                ...
                <div id="one">
                    <div class="container-demo">
                        <input type="date" name="fecha_ini" id="fecha_ini" data-mini="true" data-role="date" data-inline="true">
                        <input type="date" name="fecha_fin" id="fecha_fin" data-mini="true" data-role="date" data-inline="true">
    ...
    <script>
		$(document).ready(function () {
			$('#one > .ui-input-search').keydown(function(e) {
				if(e.keyCode == 13) {
					...
					if (buscar.length > 3) {
						...
						$.post("_operaciones/lista_ecografia.php", {tipo_operacion: "visualizar", buscar: buscar}, function (data) {
                            $("#one > table > tbody").html("");
			...
			$('#buscar').click(function(e) {
                $.post("_operaciones/lista_ecografia.php", {tipo_operacion: "visualizar", buscar: {fecha_ini: $("#fecha_ini").val(), fecha_fin: $("#fecha_fin").val()}}, function (data) {
                    $("#one > table > tbody").html("");
                    $("#one > table > tbody").append(data);
                    $("#one > table > thead").removeClass("ui-screen-hidden");
                    $("#one > table > tbody").removeClass("ui-screen-hidden");
